...
/// <summary>
/// Represents the raw and typed payload of a published UI event.
/// </summary>
public sealed class UiEventPayload(string? raw) {
    ...
    public T Deserialize<T>() {
        if (!HasValue)
            throw new InvalidOperationException("The UI event payload is empty.");

        return JsonSerializer.Deserialize<T>(Raw!, _serializerOptions)
            ?? throw new InvalidOperationException($"The UI event payload could not be deserialized to '{typeof(T).Name}'.");
    }
}
